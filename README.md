...
├─ 选项 ─────────────────────────────────────────────────────────┤
│ 图片格式: [PNG（无损）▾]  输出尺寸: [按宽度（像素）▾][800▾] 宽度固定 800 像素… │
│ 已有同名文件: [自动加序号（封面 (1)）▾]（默认不覆盖，扩展名随所选格式）        │
├─ 文件列表 ─────────────────────────────────────────────────────┤
│ 共 5 个 PDF │ 待提取 0  提取完成 5  已跳过 0  失败 0                  │
│ ┌───────────────────────┬────────────────────────────┬──────────┐ │
│ │ PDF 文件               │ 状态                        │ 所在文件夹│ │
│ ├───────────────────────┼────────────────────────────┼──────────┤ │
│ │ 樱花创造日本.pdf        │ 提取完成：800×1133           │ 【03658】…│ │
│ │ 欢乐数学.pdf           │ 提取中…                     │ 【03659】…│ │
│ │ 欢喜：女性、革命和…pdf  │ 提取完成：800×1133，另存为 … (1).jpg │ 【03660】…│ │
...
- **图片格式**：
  ...
  - `JPG（最高画质，无色度抽样）`——固定 quality=100 且 **4:4:4 不做色度抽样**，
    与原图最大偏差仅 4/255，肉眼无损。
- **输出尺寸**：四种指定方式，选完右侧会实时说明效果，转换后列表里显示每张图的实际像素：
  | 模式 | 含义 | 适合场景 |
  | --- | --- | --- |
  | 按 DPI | 按打印分辨率渲染，150 DPI 下 A4 约 1240×1754 | 存档、打印 |
  | 按宽度（像素） | 宽度固定为设定值，高度按原比例 | 网站/列表封面统一宽度 |
  | 按高度（像素） | 高度固定为设定值，宽度按原比例 | 统一行高的封面墙 |
  | 按最长边（像素） | 长边不超过设定值，等比缩放 | 统一缩略图尺寸 |

  像素模式是**直接按目标倍数渲染**（不是先渲染再缩放），字迹最锐利、无重采样损失，
  实测各种开本下宽/高/长边都精确到 1 像素不差。
- **已有同名文件**：默认「自动加序号」——`封面.png` 已存在就写成 `封面 (1).png`（扩展名随所选格式），
...
python pdf_to_png.py <PDF文件夹> [选项]
  ...
  --dpi N                 按打印分辨率输出，默认 150
  --width N               按像素宽度输出，高度按原比例
  --height N              按像素高度输出，宽度按原比例
  --max-side N            长边不超过 N 像素，等比缩放
                          （以上四个尺寸参数互斥，只能用一个）
...
# 所有封面统一宽 800 像素
python pdf_to_png.py "F:\电子书" --width 800

# 统一缩略图：长边不超过 1200 像素
python pdf_to_png.py "F:\电子书" -f jpg --max-side 1200
